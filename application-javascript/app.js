...
async function submit(args, res) {
    try {
        ...
        const userIdentity = await wallet.get("appuser27");
        if (userIdentity) {
            console.log(
                'An identity for the user "appuser27" already exists in the wallet'
            );
            return;
        }
        ...
        await wallet.put("appuser27", cx509Identity);
        console.log(
            'Successfully registered and enrolled admin user "appuser27" and imported it into the wallet'
        );
                // Check to see if we've already enrolled the user.
        const identity_user = await wallet.get("appuser27");
        if (!identity_user) {
            console.log(
                'An identity for the user "appuser27" does not exist in the wallet'
            );
...
// Get the contract from the network.
        const contract = network.getContract("voteSC");
